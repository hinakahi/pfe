        ...
        <div class="col-lg-12">
            <div id="carouselUrgent" class="carousel slide" data-bs-ride="carousel" data-bs-interval="5000">
                <div class="carousel-inner">
                    ...
                    @php
                        // ✅ Photo de fond : 1ère photo de l'annonce, sinon photo par défaut
                        $bgImage = !empty($annonce->photos)
                            ? asset('storage/'.$annonce->photos[0])
                            : asset('photo/7.jpg');
                    @endphp
                    <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                        <div class="carousel-slide-box" style="position: relative; background: url('{{ $bgImage }}') center/cover no-repeat; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.3);">
                                ...
                                @if(!empty($annonce->photos))
                                    <div class="d-flex flex-wrap gap-2 my-2">
                                        @foreach($annonce->photos as $index => $photo)
                                            <a href="{{ asset('storage/'.$photo) }}" target="_blank" class="d-block">
                                                <img src="{{ asset('storage/'.$photo) }}"
                                                     alt="Photo {{ $index + 1 }}"
                                                     class="rounded border"
...
<style>
    .carousel-slide-box { height: 320px; }
    .carousel-slide-title { font-size: 1.8rem; }

    @media (max-width: 576px) {
        .carousel-slide-box { height: 300px; }
        .carousel-slide-content { padding: 1rem 2.2rem !important; }
    ...
    .carousel-item { transition: opacity 0.8s ease-in-out !important; }
    ...
    @media (max-width: 576px) {
        ...
        .hover-card p.text-muted { font-size: 0.85rem; }
